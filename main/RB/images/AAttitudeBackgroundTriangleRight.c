#ifdef __has_include
    #if __has_include("lvgl.h")
        #ifndef LV_LVGL_H_INCLUDE_SIMPLE
            #define LV_LVGL_H_INCLUDE_SIMPLE
        #endif
    #endif
#endif

#if defined(LV_LVGL_H_INCLUDE_SIMPLE)
    #include "lvgl.h"
#else
    #include "lvgl/lvgl.h"
#endif


#ifndef LV_ATTRIBUTE_MEM_ALIGN
#define LV_ATTRIBUTE_MEM_ALIGN
#endif

#ifndef LV_ATTRIBUTE_IMG_AATTITUDEBACKGROUNDTRIANGLERIGHT
#define LV_ATTRIBUTE_IMG_AATTITUDEBACKGROUNDTRIANGLERIGHT
#endif

const LV_ATTRIBUTE_MEM_ALIGN LV_ATTRIBUTE_LARGE_CONST LV_ATTRIBUTE_IMG_AATTITUDEBACKGROUNDTRIANGLERIGHT uint8_t AAttitudeBackgroundTriangleRight_map[] = {
  , 
};

const lv_img_dsc_t AAttitudeBackgroundTriangleRight = {
  .header.cf = LV_IMG_CF_ALPHA_1BIT,
  .header.always_zero = 0,
  .header.reserved = 0,
  .header.w = 119,
  .header.h = 52,
  .data_size = 780,
  .data = AAttitudeBackgroundTriangleRight_map,
};
